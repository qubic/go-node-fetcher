...
func run() error {
	var cfg struct {
		...
		Qubic struct {
			TickErrorThreshold int      `conf:"default:50"`
	...
	rp, err := NewPeers(
		cfg.Qubic.FixedPeerList,
		cfg.Qubic.StartingPeersIP,
		cfg.Qubic.WhitelistPeers,
		cfg.Qubic.MaxPeers,
		cfg.Qubic.ExchangeTimeout,
		cfg.Qubic.TickErrorThreshold,
		db)
	if err != nil {
